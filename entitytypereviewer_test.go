...
func TestEntityTypeReviewerList(t *testing.T) {
	...
	_, err := client.EntityTypes.Reviewers.List(context.TODO(), "typeID")
	if err != nil {
...
func TestEntityTypeReviewerAssign(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
...
func TestEntityTypeReviewerRemove(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
